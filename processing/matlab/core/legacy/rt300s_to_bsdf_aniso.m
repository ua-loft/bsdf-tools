...
%   8) Redundancy of scatter along surface normal is removed by assuming 
%      FRED only needs entries for: \phi_s=0 at \theta_s=0.
%
% Developer(s):
%   - Jacob P. Krell (JPK)
...
% | 2026.03.16 | JPK  | Initialized support for FRED format by mostly     |
% |            |      | copying code from isotropic processing script,    |
% |            |      | but modifying where applicable for anisotropic;   |
% 
% References:
...
if OUTPUT_TO_FRED
    % First, average (A=+90,-R) data into (A=-90,+R), because function
    % 'save_as_fred_aniso' assumes A \in [-90,90):
    mA_1 = A == -90;
    mA_2 = A == 90;
    RT_to_func = RT; % copy, to preserve 'RT' unchanged
    for i = 1:nI
        mAI_1 = and(mA_1, I == Iu(i));
        mAI_2 = and(mA_2, I == Iu(i));
        for k = 1:nR
            mAIR_1 = and(mAI_1, R == Ru(k));
            mAIR_2 = and(mAI_2, R == -Ru(k));
            for m = 1:M
                % Average (A=+90,-R) into (A=-90,+R):
                RT_to_func{m}(mAIR_1) = (RT{m}(mAIR_1) + RT{m}(mAIR_2)) / 2;
            end
        end
    end
    % Remove A=+90 from all vectors:
    mA = A ~= 90;
    I_to_func = I(mA);
    A_to_func = A(mA);
    R_to_func = R(mA);
    BRDF_in_RT300S_frame = cell(size(RT_to_func));
    for m = 1:M
        RT_to_func{m} = RT_to_func{m}(mA);
        BRDF_in_RT300S_frame{m} = RT_to_func{m} ./ (pi * cosd(R_to_func)); % equation 4.6 of [2]
    end
    % Pass to function to save as FRED file:
    save_as_fred_aniso(I_to_func, A_to_func, R_to_func, sample_rotations, ...
...
function status = save_as_fred_aniso(I, A, R, rot, ... 
    BRDF, name_txt_file)
    % Assumption(s):
    %   1) 'A' \in [-90,90), and if full [-90,90] was measured then
    %      (A=-90,+R) and (A=+90,-R) were averaged outside this function.
    %   2) 'BRDF' is cell of size (M,1), and
    %      'rot' is vector also of size (M,1) with 
    %      corresponding RT-300S stage rotation angles.
            ...
            for k = 1:length(theta_s)
                ...
                for R_sign = [+1, -1] % for sign of what R actually is
                    ...
                    for Ai = 1:nA % for \phi_s
                        ...
                        if sum(mIRA) ~= 1
                            error("Was expecting single true value " + ...
                                "in mask. Debug code manually.")
                        end
    ...
    for Ii = 1:nI % for \theta_i
        ...
        % Define all incident azimuths for anisotropic:
        for rotj = 1:length(rot) % for \phi_i
            ...
            % Write for \theta_s = 0:
            BRDF_at_R0 = BRDF{rotj}(mRI); % average values from all 'A'
